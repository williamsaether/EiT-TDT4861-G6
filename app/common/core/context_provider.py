...
from app.common.providers.speed_limit_provider import SpeedLimitProvider
from app.common.providers.weather_provider import WeatherProvider
...
class RoadContextProvider:
    """Context provider used by both web and CLI flows."""

    def __init__(self) -> None:
        self.reference_points = self._load_metadata()
        self.speed_limit_provider = SpeedLimitProvider()
    ...
    def get_context(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        weather = self.weather_provider.lookup(lat, lon)
        ref = self._nearest_reference(lat, lon)
        speed_lookup = self.speed_limit_provider.lookup(lat, lon)
